...
exits without calling the network. With the key set it runs ownership
matching (name + street address) or a California company search.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--name", default=example["name"], help="Business or company name")
    ...
    parser.add_argument("--limit", type=int, default=5, help="Company-search page size")
    parser.add_argument(
        "--credits",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.credits:
        ...
        return 0
    if args.mode == "search":
        print(json.dumps(search_california_companies(client, args.name, limit=args.limit), indent=2))
        return 0
